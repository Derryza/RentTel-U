...
public class HelperUser {
    ...
    public HelperUser(SQLiteDatabase dbparam){
        this.Activedb = dbparam;
    }


    public long createUser(User todo, long[] tag_ids) {
        SQLiteDatabase db = this.Activedb;

        ContentValues values = new ContentValues();
        values.put(KEY_ID_USER, todo.getId_user());
        values.put(KEY_ALAMAT, todo.getAlamat());
        values.put(KEY_NAMA, todo.getNama());
